...
export default function EditTuition() {
  ...
  if (error) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-bg">
        <div className="text-center">
          <FiAlertCircle className="text-5xl text-error mx-auto mb-4" />
      ...
      <div className="w-full rounded-2xl">
        <div className="mb-8">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
           ...
           <button
  onClick={handleCancel}
  className="btn-outline inline-flex items-center justify-center gap-2 px-4 py-2 rounded-md border border-gray-300 text-gray-700 hover:bg-gray-100 transition-colors duration-200"
>
  <FiX className="text-lg" />
  <span>Cancel</span>
</button>
          </div>
          <p className="text-text-secondary">Update your tuition information. Make sure all required fields are filled.</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="form-group">
                  ...
                  {errors.grade && <p className="text-error text-sm mt-1">{errors.grade.message}</p>}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="form-group">
                  ...
                  <div className="space-y-2 max-h-48 overflow-y-auto p-2 border border-border rounded bg-card-bg">
                    {availableSubjects.map((subj) => (
                      <label key={subj} className="flex items-center gap-2 cursor-pointer p-1 hover:bg-hover-bg rounded">
                        <input 
                          type="checkbox" 
                          value={subj}
                          checked={subjects?.includes(subj)}
                          onChange={(e) => handleSubjectsChange(subj, e.target.checked)}
                          className="rounded text-primary focus:outline-none"
                        />
                        <span className="text-sm text-text-primary">{subj}</span>
                      </label>
                    ))}
                  </div>
                  {errors.subjects && <p className="text-error text-sm mt-1">{errors.subjects.message}</p>}
                </div>
                ...
                <div className="form-group">
                  ...
                  {errors.tuitionType && <p className="text-error text-sm mt-1">{errors.tuitionType.message}</p>}
                </div>
                ...
                <div className="form-group">
                  ...
                  {errors.totalFee && <p className="text-error text-sm mt-1">{errors.totalFee.message}</p>}
                </div>
              </div>
          ...
          {/* Location Information (if not online) */}
          {tuitionType !== "online" && (
            <div className="bg-card-bg rounded-lg shadow p-6">
              <h2 className="text-xl font-semibold text-text-primary mb-6 flex items-center gap-2">
                <FiMapPin className="text-primary" /> Location Information
              </h2>
          ...
          <div className="bg-card-bg rounded-lg shadow p-6">
            <h2 className="text-xl font-semibold text-text-primary mb-6 flex items-center gap-2">
              <FiCalendar className="text-primary" /> Schedule Proposals (Optional)
            </h2>
            
            {fields.map((proposal, pIndex) => (
              <div key={proposal.proposalKey} className="border border-border rounded-lg p-4 mb-4">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-2">
                  <h3 className="font-medium text-text-primary">Schedule Proposal {pIndex + 1}</h3>
                  <button 
                    type="button" 
                    className="text-error border border-error/20 px-3 py-1 rounded hover:bg-error/10"
                    onClick={() => remove(pIndex)}
                  >
                    Remove Proposal
                  </button>
                ...
                <div className="space-y-4">
                  ...
                  {watch(`scheduleProposals.${pIndex}.schedule`)?.map((schedule, sIndex) => (
                    <div key={sIndex} className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end p-3 bg-hover-bg rounded-lg">
                      <div className="form-group mb-0">
                        <label className="form-label text-sm">Day</label>
                        <select 
                          className="form-input w-full"
                          {...register(`scheduleProposals.${pIndex}.schedule.${sIndex}.day`, { valueAsNumber: true })}
                        >
                      ...
                      <div className="form-group mb-0">
                        <label className="form-label text-sm">From</label>
                        <input 
                          type="time" 
                          className="form-input w-full"
                          {...register(`scheduleProposals.${pIndex}.schedule.${sIndex}.from`)} 
                      ...
                      <div className="form-group mb-0">
                        <label className="form-label text-sm">To</label>
                        <input 
                          type="time" 
                          className="form-input w-full"
                          {...register(`scheduleProposals.${pIndex}.schedule.${sIndex}.to`)} 
                      ...
                      <button 
                        type="button" 
                        className="text-error border border-error/20 px-3 py-2 rounded hover:bg-error/10"
                        onClick={() => removeScheduleSlot(pIndex, sIndex)}
                      >
                        Remove Slot
                      </button>
